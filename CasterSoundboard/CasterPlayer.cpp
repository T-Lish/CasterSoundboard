...
CasterPlayerWidget::CasterPlayerWidget(QWidget* parent) : QWidget(parent)
{
    ...
    connect(player,SIGNAL(playbackStateChanged(QMediaPlayer::PlaybackState)),this,SLOT(playerStateChanged(QMediaPlayer::PlaybackState)));
...
{
    //Update Volume
    OscMessageComposer* msg1 = writeOSCMessage("/cbp/" + this->id->toUtf8() + "/m/vol", volumeSlider->value() );
...
void CasterPlayerWidget::volumeChanged(int value)
{
    ...
    if(isAudioDucked)
      ...
      audioOutput->setVolume(playerState->volume * 0.0033);
    else
      //player->setVolume(playerState->volume);
      audioOutput->setVolume(playerState->volume * 0.01);

    //Update OSC Client
    OscMessageComposer* msg = writeOSCMessage("/cbp/" + this->id->toUtf8() + "/m/vol", playerState->volume );
